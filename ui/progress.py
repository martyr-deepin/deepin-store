...
class GtkAptProgress(gtk.VBox):
    ...
    def clear(self):
        ...
        self._progressbar.progress_buffer.progress = 0
    ...
    def _on_status_changed(self, progress, status, percent):
        ...
        if percent is None or percent == -1:
            ...
            self._progressbar.progress_buffer.progress = percent
        while gtk.events_pending():
            gtk.main_iteration()
...
def _test():
    dialog = DialogBox("Progress Demo", close_callback=gtk.main_quit)
    dialog.set_size_request(400, -1)
    progress = GtkAptProgress()
    dialog.body_box.pack_start(progress)
    progress.show()
    cache = apt.cache.Cache(progress.open)
    
    #progress.show_terminal(expanded=True)
    dialog.show_all()
    cache.update(progress.acquire)
    gtk.main()
